...
# Inline entry workflow (Weekly Controls, Key Notes, Media Summaries, Injuries, Opponent Preview, Predictions)
# PLUS: Auto Snap Counts button in sidebar with default set to CHI and current NFL week
...
st.caption("Weekly controls + inline sections restored • Auto Snap Counts in sidebar • Upload CSVs right below Weekly Controls • Use 3‑letter opponent codes (e.g., MIN).")
...
TEAM_CODES = ["ARI","ATL","BAL","BUF","CAR","CHI","CIN","CLE","DAL","DEN","DET","GB","HOU","IND","JAX","KC","LAC","LAR","LV","MIA","MIN","NE","NO","NYG","NYJ","PHI","PIT","SEA","SF","TB","TEN","WAS"]
...
def compute_dvoa_proxy(off_df: pd.DataFrame, def_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not off_df.empty and not def_df.empty and all(c in off_df.columns for c in ["Week","Opponent"]) and all(c in def_df.columns for c in ["Week","Opponent"]):
        ...
        base["YPA"] = base.get("YPA_OFF"); base["CMP%"] = base.get("CMP%_OFF"); base["SR%"] = base.get("SR%_OFF")
        base["YAC"] = base.get("YAC_OFF"); base["Penalties"] = base.get("Penalties_OFF"); base["Penalty_Yards"] = base.get("Penalty_Yards_OFF")
        base["RZ%_Allowed"] = base.get("RZ%_Allowed_DEF"); base["SACKs"] = base.get("SACKs_DEF"); base["Pressures"] = base.get("Pressures_DEF")
        base["INTs"] = base.get("INTs_DEF"); base["YAC_Allowed"] = base.get("YAC_Allowed_DEF")
    else:
        base = off_df.copy() if not off_df.empty else def_df.copy()
    if base.empty:
        return pd.DataFrame(columns=["Week","Opponent","DVOA_Proxy"])
    def score_row(row):
        ypa = safe_num(row.get("YPA", math.nan)); cmpct = safe_num(row.get("CMP%", math.nan)); sr = safe_num(row.get("SR%", math.nan))
        yac = safe_num(row.get("YAC", math.nan)); pen = safe_num(row.get("Penalties", math.nan)); pen_yards = safe_num(row.get("Penalty_Yards", math.nan))
        rz_allowed = safe_num(row.get("RZ%_Allowed", math.nan)); sacks = safe_num(row.get("SACKs", math.nan)); pressures = safe_num(row.get("Pressures", math.nan))
        ints = safe_num(row.get("INTs", math.nan)); yac_allowed = safe_num(row.get("YAC_Allowed", math.nan))
...
# =========================
# Sidebar (includes Auto Snap Counts)
# =========================
with st.sidebar:
    st.header("Controls")
    ...
    if st.button("Fetch Snap Counts (Auto)"):
        ...
        if added:
            st.success(f"Added {added} Snap_Counts rows for {sc_team}, Week {sc_week}.")
    st.divider()
...
# =========================
# Browse & Add Files (now directly under Weekly Controls)
# =========================
st.subheader("📎 Browse & Add Files")
st.caption("Drop CSV/XLSX files to append data. We'll normalize Week & Opponent and de‑duplicate smartly.")

def _up(lbl, sheet, req_cols, dedup):
    up = st.file_uploader(lbl, type=["csv","xlsx"], key=f"{sheet}_up_inline")
    if up is not None:
        try:
            df = pd.read_csv(up) if up.name.lower().endswith('.csv') else pd.read_excel(up)
        except Exception as e:
            st.error(f"Failed to read: {e}"); df = pd.DataFrame()
        if not df.empty:
...
with _col1:
    _up("Offense file", "Offense", ["Week","Opponent","YPA","CMP%","SR%","Points","Penalties","Penalty_Yards","YAC"], ["Week","Opponent"])
    _up("Personnel file", "Personnel", ["Week","Opponent","11","12","13","21","Other"], ["Week","Opponent"])
with _col2:
    _up("Defense file", "Defense", ["Week","Opponent","RZ%_Allowed","SACKs","Pressures","INTs","Penalties","Penalty_Yards","YAC_Allowed"], ["Week","Opponent"])
    _up("Snap Counts file", "Snap_Counts", ["Week","Opponent","Player","Snaps","Snap%","Side"], ["Week","Opponent","Player","Side"])

# =========================
# Inline Sections (Key Notes, Media, Injuries, Opponent Preview, Predictions)
# =========================
...
if st.button("Export Final PDF"):
    if FPDF is None:
        ...
        def add_table(title, df):
            ...
            if df is None or df.empty: pdf.cell(0, 6, txt="(no data)", ln=True); return
            cols = list(df.columns)[:6]; pdf.cell(0, 6, txt=", ".join(cols), ln=True)
            for _, r in df[cols].head(25).iterrows():
                row_txt = ", ".join([str(r[c]) for c in cols]); pdf.cell(0, 5, txt=row_txt[:120], ln=True)
...
st.divider()
st.info("CSV headers: Offense: Week,Opponent,YPA,CMP%,SR%,Points,Penalties,Penalty_Yards,YAC | Defense: Week,Opponent,RZ%_Allowed,SACKs,Pressures,INTs,Penalties,Penalty_Yards,YAC_Allowed | Personnel: Week,Opponent,11,12,13,21,Other | Snap_Counts: Week,Opponent,Player,Snaps,Snap%,Side.")
