...
def _calc_proxy(off_row: pd.Series=None, def_row: pd.Series=None) -> float | None:
    ...
    count = 0
    def val(s, k): 
        try: return float(s.get(k)) if s is not None and k in s else None
        except: return None
...
with cols_wc[0]: selected_week = st.number_input("Week",min_value=1,step=1,value=1)
with cols_wc[1]: selected_team = st.text_input("Team",value="CHI")
with cols_wc[2]: opponent = st.text_input("Opponent",value="TBD")
with cols_wc[3]: key_notes = st.text_area("Key Notes (appear in PDF)")

# ========= Uploads (Off/Def/etc.) =========
st.markdown("### Upload Weekly Data")
cols_up = st.columns(4)
with cols_up[0]: off_file = st.file_uploader("Offense CSV", type=["csv"])
with cols_up[1]: def_file = st.file_uploader("Defense CSV", type=["csv"])
with cols_up[2]: pers_file = st.file_uploader("Personnel CSV", type=["csv"])
with cols_up[3]: snaps_file = st.file_uploader("Snap Counts CSV", type=["csv"])

def _load_csv_to_df(f,week:int):
    if not f: return None
    try:
        df = pd.read_csv(f)
        if "Week" not in df.columns: df.insert(0,"Week",week)
        else: df["Week"]=week
        return df
    except Exception as e:
        st.error(f"CSV load error: {e}"); return None
...
if st.button("Save Weekly Uploads"):
    for name,f in [("Offense",off_file),("Defense",def_file),("Personnel",pers_file),("SnapCounts",snaps_file)]:
        ...
        if df is not None: _append_df(name,df,["Week"])
...
st.write(f"Week {selected_week} proxy: {proxy_val if proxy_val else '—'}")

# ========= YTD Summary =========
st.markdown("### YTD Summary (auto)")
def _ytd(df,week:int): return df[df["Week"].between(1,week)] if not df.empty else pd.DataFrame()
...
if off_ytd.empty and def_ytd.empty:
    ...
    if not off_ytd.empty: merged["Team"]=off_ytd.mean(numeric_only=True).reindex(METRIC_SCHEMA.keys())
    if not def_ytd.empty: merged["NFL"]=def_ytd.mean(numeric_only=True).reindex(METRIC_SCHEMA.keys())
    st.dataframe(_colorize_df(merged),use_container_width=True)

# ========= Exports (Excel/PDF placeholders) =========
st.markdown("### Exports")
if st.button("Export Pre (Excel)"): st.info("Would create Pre Excel here.")
if st.button("Export Pre (PDF)"):
    if not HAS_REPORTLAB: st.warning("Install reportlab for PDF export")
    else: st.info("Would create Pre PDF here.")
