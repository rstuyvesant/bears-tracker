...
def _clean_week(df: pd.DataFrame) -> pd.DataFrame:
    """Coerce Week to int and drop anything < 1."""
    try:
        if isinstance(df, pd.DataFrame) and "Week" in df.columns:
            wk = pd.to_numeric(df["Week"], errors="coerce").fillna(0).astype(int)
            df = df.copy()
            df["Week"] = wk
            df = df[df["Week"] >= 1]
    except Exception:
...
# --------------------------
# Append new data to Excel workbook (with cleanup and robust deduping)
# --------------------------
    ...
    try:
        ...
        if os.path.exists(file_name):
            ...
            if sheet_name in book.sheetnames:
                ...
                existing_data = _clean_week(existing_data)
                # Deduplicate by Week if requested
                if deduplicate and "Week" in existing_data.columns and "Week" in new_data.columns:
                    new_weeks = set(pd.to_numeric(new_data["Week"], errors="coerce").dropna().astype(int).tolist())
                    if len(existing_data) > 0:
                        existing_data["Week"] = pd.to_numeric(existing_data["Week"], errors="coerce").fillna(0).astype(int)
                        existing_data = existing_data[~existing_data["Week"].isin(new_weeks)]
        ...
        # Final clean and sort by Week if available
        combined_data = _clean_week(combined_data)
        if "Week" in combined_data.columns:
            combined_data = combined_data.sort_values(by=["Week"], kind="stable").reset_index(drop=True)
        ...
        if isinstance(combined_data, pd.DataFrame) and not combined_data.empty:
            ...
            # If empty but we have columns, still write headers
            if isinstance(combined_data, pd.DataFrame) and len(combined_data.columns) > 0:
                sheet.append(list(combined_data.columns))
...
if uploaded_strategy:
    ...
    df_strategy = _clean_week(df_strategy)
...
if uploaded_personnel:
    ...
    df_personnel = _clean_week(df_personnel)
...
# --------------------------
# OPTIONAL: Fetch weekly team data via nfl_data_py (basic Offense/Defense rows)
# --------------------------
    ...
    if st.button("Fetch CHI Week via nfl_data_py"):
        try:
            ...
            if team_week.empty:
                ...
                pass_yards_series = team_week.get("passing_yards")
                pass_yards = pass_yards_series.values[0] if pass_yards_series is not None else None
...
if st.sidebar.button("Fetch Play-by-Play Metrics"):
    try:
        ...
        if pbp_w.empty:
            ...
            # Red Zone % Allowed: min yardline per drive
            dmins = (
            ...
            metrics_df = _clean_week(metrics_df)
            ...
            st.caption("Note: Hurries are not separately flagged in standard PBP; pressures = sacks + QB hits.")
    except Exception as e:
...
if st.sidebar.button("Compute DVOA-like Proxy"):
    try:
        ...
        if bears_off.empty and bears_def.empty:
            ...
            # Opponent adjustments (differences)
            def safe_diff(a, b):
...
def _read_sheet(name):
    ...
    try:
        ...
        return _clean_week(df)
    except Exception:
        return pd.DataFrame()

df_offense_prev   = _read_sheet("Offense")
df_defense_prev   = _read_sheet("Defense")
df_strategy_prev  = _read_sheet("Strategy")
df_personnel_prev = _read_sheet("Personnel")
df_dvoa_prev      = _read_sheet("DVOA_Proxy")

if not df_offense_prev.empty:
    st.subheader("Offensive Analytics")
    st.dataframe(df_offense_prev)
if not df_defense_prev.empty:
    st.subheader("Defensive Analytics")
    st.dataframe(df_defense_prev)
if not df_strategy_prev.empty:
    st.subheader("Weekly Strategy")
    st.dataframe(df_strategy_prev)
if not df_personnel_prev.empty:
    st.subheader("Personnel Usage")
    st.dataframe(df_personnel_prev)
if not df_dvoa_prev.empty:
    st.subheader("📊 DVOA-like Proxy Metrics (last 5)")
    st.dataframe(df_dvoa_prev.sort_values("Week").tail(5))
...
# --------------------------
# Prediction Section (uses Strategy + Offense/Defense + Advanced_Defense + DVOA_Proxy when available)
# --------------------------
...
if os.path.exists(EXCEL_FILE):
    try:
        ...
        if not row_s.empty and not row_o.empty and not row_d.empty:
            ...
            prediction_entry = _clean_week(prediction_entry)
